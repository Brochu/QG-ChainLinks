...
### Status: **deferred, post-launch nice-to-have** · brainstormed 2026-07-06 · no impact on the slice or the v2.1 format

> Dark Souls-inspired deferred multiplayer: rare traces of other players' judgment
> surfacing in your case. No real-time multiplayer, ever. The game must be fully
> playable — mechanically identical — with this feature absent or disabled.
...
which. Following it or ignoring it is the same gamble as any testimony — that's the
point, and it's the same doubt muscle the rest of the game trains.
...
- **Overlay, never facts.** A note never enters the discovered set, carries no tags,
  ...
  derived noun universe must stay pure). The rules engine never sees notes; they are
  presentation-side only. This is what makes the on/off toggle trivial.
- **No freeform text anywhere.** A note is auto-derived from another player's
  judgment tag: the shared record is just `{tag_type, target_id, block_index}`.
  The reader's game renders it through small authored phrase pools per tag type,
  plus a generated period-flavored agent name. No text travels → nothing to moderate.
- **No composition step for the writer.** Players just play and tag as normal;
  behind a consent toggle, their tags feed the pool. Writing for yourself and
  "writing" for strangers are the same gesture.
- **Rare by design: ~1 note per 5–10 cases.** An event, not an information layer.
  ...
  needed; cold start is a non-problem at this frequency.
...
- **Sample honestly.** The candidate tag is drawn randomly across eligible tags —
  color facts and dead-end suspects included. If notes only ever pointed at
  load-bearing facts, receiving one would itself be a hint.
- **No metadata that becomes an oracle.** No writer case outcome, no ratings, no
  upvotes, no seniority. Every note arrives with identical, unearnable authority.
- **No mechanical bonus/malus.** Time is the only resource; a bad note costs blocks
  organically, a good one saves them. Nothing else needed — and at this rarity there
  is nothing to balance anyway.
...
- **Writer-side echo:** a one-line beat next session — *"your assessment on the
  Ashford matter was circulated to another field office"* — faint two-way warmth,
  Death Stranding-style. One extra field + one pager line; cuttable.
...
attachments as a poor-man's record store), with the consent toggle mapping onto a
normal in-game option. Worth a proper SDK survey when the time comes; the design
above deliberately requires nothing more than "upload tiny record / fetch a few
random records for case X."
