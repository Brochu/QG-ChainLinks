...
void UAlibiTxtBasedCheats::AddedToCheatManager_Implementation() {
	case_system = UGameplayStatics::GetGameInstance(GetWorld())->GetSubsystem<UCaseSubsystem>();
}
...
void UAlibiTxtBasedCheats::ShowCaseMeta() {
	const FCaseMetadata &meta = case_system->file.meta;
...
void UAlibiTxtBasedCheats::CaseStatus() {
	const FCaseMetadata &meta = case_system->file.meta;
	const FCaseSaveState &save = case_system->save;
	...
	for (const FPendingRequest &req : save.active_lab_requests) {
		...
		const FCaseAction *act = case_system->file.actions.FindByPredicate(
...
void UAlibiTxtBasedCheats::ListLocations() {
	MessageConsole(FString::Format(TEXT("All available locations in the case: {0}"), { *case_system->file.meta.title.ToString()}));

	TArray<int32> location_idx = case_system->list_location_idx();
	for (int32 i = 0; i < location_idx.Num(); i++) {
		FCaseLocation &loc = case_system->file.locations[location_idx[i]];
...
void UAlibiTxtBasedCheats::ExploreLocation() {
	const FCaseLocation &loc = case_system->get_active_location();
	MessageConsole(FString::Format(TEXT("All available actions at {0}"), { *loc.name.ToString() }));

	TArray<int32> action_idx = case_system->list_action_idx(loc.location_id);
	for (int32 i = 0; i < action_idx.Num(); i++) {
		FCaseAction &act = case_system->file.actions[action_idx[i]];

		if (case_system->action_visibility(act) == EActionVisibility::Locked) {
...
void UAlibiTxtBasedCheats::MoveToLocation(int32 new_location_idx) {
	...
	FName target_loc_id = case_system->file.locations[location_idx[new_location_idx]].location_id;
	if (!case_system->move_location(target_loc_id)) {
...
void UAlibiTxtBasedCheats::ChooseAction(int32 action_idx) {
	// Index-based because it's easier to type on cli. Will be ID-based in actual game with real UX
	const FCaseLocation &loc = case_system->get_active_location();
	TArray<int32> actions_at_location = case_system->list_action_idx(loc.location_id);
	...
	FCaseAction &act = case_system->file.actions[actions_at_location[action_idx]];
	...
	const ECommitActionResult res = case_system->commit_action(act.action_id);
...
void UAlibiTxtBasedCheats::MessageConsole(FString message) {
	GetPlayerController()->ClientMessage(message);
